...
where m.deptno = e.deptno and m.minsal = e.sal
order by deptno
;
...
--mgr에 empno가 없는 직원이 부하직원이 없는 사원이다.
select ename
from emp
...
--mgr에 empno가 있는 사람이 부하직원이 있는 사원이다.
...
--부서번호 30에 속하는 사원의 이름과 입사일을 표시하라
select ename, hiredate
from emp
where deptno = (select deptno from emp where ename = 'BLAKE') and ename != 'BLAKE'
;

--52. 급여가 평균 급여보다 많은 사원들의 사원 번호와 이름을 표시
--하되 결과를 급여에 대해서 오름차순으로 정렬하시오.
select empno, ename
from emp
...
select empno, ename
from emp
where job IN (select job from emp where ename like '%K%')
;
...
SELECT EMPNO, ENAME, SAL
FROM EMP
WHERE DEPTNO IN(SELECT DEPTNO FROM EMP WHERE SAL>(SELECT AVG(SAL) FROM EMP) AND ENAME LIKE '%M%')
;
